...
from baselines.steering2.config2 import DEVICE, LOG_DIR, TARGET_LAYER
...
def _plot_metric_trends(history: List[dict], split: str) -> Optional[Path]:
    ...
    plot_path = Path(LOG_DIR) / f"baseline_softmax_{split}_metrics.png"
...
def evaluate_softmax_baseline(split: str = "validation", csv_name: str = "baseline_softmax.csv"):
    ...
    logger = setup_logger("baseline_softmax", f"baseline_softmax_{split}.log")

    logger.info(f"Loaded MedQA {split} dataset with {len(dataset)} samples.")
    logger.info("Loaded model: Qwen/Qwen2.5-3B-Instruct")
    logger.info(f"Starting softmax baseline evaluation for {len(dataset)} MedQA samples...")
